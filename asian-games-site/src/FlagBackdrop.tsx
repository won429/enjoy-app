...
export default function FlagBackdrop(){
 ...
 useEffect(()=>{
  ...
  const resize=()=>{renderer.setSize(host.clientWidth,host.clientHeight);material.uniforms.uAspect.value=host.clientWidth/host.clientHeight;};resize();const observer=new ResizeObserver(resize);observer.observe(host);
  const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;let visible=false,raf=0,last=0;
  const loop=(time:number)=>{raf=requestAnimationFrame(loop);if(!visible||document.hidden||time-last<32)return;last=time;material.uniforms.uTime.value=reduced?0:time*.001;renderer.render(scene,camera);};
  ...
  return()=>{cancelAnimationFrame(raf);observer.disconnect();visibility.disconnect();geometry.dispose();material.dispose();texture.dispose();renderer.dispose();renderer.domElement.remove();};
 },[]);
